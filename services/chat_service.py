...
2. **PPT**：用 Slidev Markdown 格式生成幻灯片。遵循以下规则：
   - 使用 `---` 分隔每页幻灯片，第一行必须是 `---`
   - 每页使用 Markdown 语法（标题、列表、代码块、表格、引用等）
   - 第一页使用 `layout: cover` 作为封面页
   - 可以使用 `layout: center`、`layout: fact`、`layout: quote` 等布局
   - 不要包含任何 HTML 代码，只使用纯 Slidev Markdown 语法
   - 在回复末尾用标记 [ppt] 包裹，格式为：
   先写一段说明文字，然后换行后写：
   [ppt]\n（完整的 Slidev Markdown 内容）\n[/ppt]
